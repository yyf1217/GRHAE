...
def _mobius_addition_batch(x, y, c):
    ...
    res = num / (denom.unsqueeze(2) + 1e-10)
    return res
...
def tf_project_hyp_vecs(x, c):
    ...
    return x * ((1. - PROJ_EPS) / np.sqrt(c)) / torch.norm(x)
    
    
def mob_scalar_mul(r, v, c=1.0):
    EPS = 1e-10
    v = v + EPS
    norm_v = torch.norm(v)
    
    nomin = torch.tanh(r * torch.atanh(np.sqrt(c) * norm_v))
...
# klein
def _dist_matrix_klein(x, y, c, eps=1e-5):
    xy = _tensor_dot(x, y)  # B x C
    x2 = x.pow(2).sum(-1, keepdim=True)  # B x 1
    y2 = y.pow(2).sum(-1, keepdim=True)  # C x 1
    num = 1 - xy  # B x C
    denom = torch.sqrt(1 - x2 + eps) * torch.sqrt(1 - y2.permute(1, 0) + eps)  # B x C
    cosh_arg = num / (denom + eps)
    distance = torch.acosh(cosh_arg.clamp(min=1.0))  # Ensure valid range for acosh
    return distance
 
def dist_matrix_klein(x, y, c=1.0):
    c = torch.as_tensor(c).type_as(x)
    return _dist_matrix_klein(x, y, c)
