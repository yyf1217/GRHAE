...
class HNN_gram(BaseDetector):
    ...
    def fit(self, loader, in_feats, y_true=None):
        ...
        for cur_epoch in range(self.epoch):
            ...
            for data in loader:
                ...
                # Equation (7): c_X=1/(1+||x_i-x_j||^2) and
                # c_H=1/(1+d_{H,-c}(h_i,h_j)^2).  Passing c explicitly is
                # essential because the manifold helper otherwise defaults
                # to c=1 even though this network normally uses c=0.2.
                x_distance_squared = torch.cdist(x, x, p=2).pow(2)
                h_distance_squared = self.manifold.pairwise_distance(
                    h, c=self.c
                ).pow(2)
    ...
    def decision_function(self, data):
        ...
        # Geoopt's k is signed sectional curvature.  The model stores the
        # positive magnitude c of a Poincare ball with curvature -c.
        k = scores.new_tensor(-self.c)
        # Manuscript Eq. (10): p_j = proj_B(h * a_j).
